...
class Derivatives(Scene):
    def construct(self):
        ...
        func1_lab = MathTex("f(x)=\\frac{1}{3} {x}^{3}").set(width=2.5).next_to(plane1, UP, buff=0.2).set_color(RED_C)
        ...
        dot = always_redraw(
            lambda: Dot().move_to(plane1.c2p(k.get_value(), func1.underlying_function(k.get_value())))
        )
        ...
        func2 = always_redraw(lambda: plane2.plot(lambda x: x**2, x_range=[-3,k.get_value()], color=GREEN))
        func2_lab = MathTex("f'(x)={x}^{2}").set(width=2.5).next_to(plane2, UP, buff=0.2).set_color(GREEN)
